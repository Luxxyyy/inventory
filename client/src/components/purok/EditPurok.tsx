...
const EditPurok = () => {
  ...
  // Fetch Balangay & Source lists
  const fetchSourcesAndBalangays = async () => {
    try {
      const [srcList, bgyList] = await Promise.all([getSources(), getBalangays()]);
      setSources(srcList);
      setBalangays(bgyList);
    } catch {
                ...
                {puroks.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td className="text-center">
                      <button className="btn btn-sm btn-success me-2 text-white" onClick={() => openEditModal(p)}>
                        Edit
                      </button>
                      <button className="btn btn-sm btn-danger text-white" onClick={() => setDeletePurokItem(p)}>
                        Delete
                      </button>
                    </td>
                  </tr>
      ...
      {selectedPurok && (
        <Modal onClose={closeModal} title="Edit Purok">
          <div className="mb-3">
            ...
            <input name="purok" className="form-control" value={editForm.purok} onChange={handleInputChange} />
          </div>
          <div className="mb-3">
            <label className="form-label">Latitude</label>
            <input name="latitude" className="form-control" value={editForm.latitude} onChange={handleInputChange} />
          </div>
          <div className="mb-3">
            <label className="form-label">Longitude</label>
            <input name="longitude" className="form-control" value={editForm.longitude} onChange={handleInputChange} />
          </div>
          <div className="mb-3">
            <label className="form-label">Balangay</label>
            <select name="balangay_id" className="form-select" value={editForm.balangay_id} onChange={handleInputChange}>
              <option value="">Select Balangay</option>
              {balangays.map((b) => (
                <option key={b.id} value={b.id}>{b.balangay}</option>
              ))}
            </select>
          </div>
          <div className="mb-3">
            <label className="form-label">Source</label>
            <select name="source_id" className="form-select" value={editForm.source_id} onChange={handleInputChange}>
          ...
          <div className="d-flex justify-content-end">
            ...
            <button className="btn btn-success text-white" onClick={handleUpdate}>Save</button>
          </div>
        </Modal>
      )}
      {deletePurokItem && (
        <Modal onClose={() => setDeletePurokItem(null)} title="Confirm Delete">
          <p>Are you sure you want to delete <strong>{deletePurokItem.purok}</strong>?</p>
          <div className="d-flex justify-content-end">
            <button className="btn btn-primary me-2" onClick={() => setDeletePurokItem(null)}>Cancel</button>
            <button className="btn btn-danger text-white" onClick={handleDelete}>Delete</button>
          </div>
        </Modal>
      )}
    </div>
